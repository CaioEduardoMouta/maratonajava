...
import javacore.ZZKstreams.classes.Genero;
...
public class StreamTest6Collectors2 {
    public static void main(String[] args) {
        ...
        List<Pessoa> bom = new ArrayList<>();
        List<Pessoa> ruim = new ArrayList<>();
        for (Pessoa pessoa: pessoas) {
            if(pessoa.getGenero().equals(Genero.RUIM))
                ruim.add(pessoa);
            else
                bom.add(pessoa);
        }
        generoListMap.put(Genero.RUIM, ruim);
        generoListMap.put(Genero.BOM, bom);
      ...
      //  Map<Genero, Map<Maioridade, List<Pessoa>>
       ...
       Map<Genero, Long> collect3 = pessoas.stream()
        ...
        //Agrupando por genero e maior salário
